...
namespace BalabobaDiscordBot.Api
{
    public class Categories : IChoiceProvider
    {
        public async Task<IEnumerable<DiscordApplicationCommandOptionChoice>> Provider()
        {
            ...
            foreach (var i in (await BalabobaRequests.GetBalabobaIntros("https://yandex.ru/lab/api/yalm/intros_eng")).Intros!)
            {
                Choices.Add(new DiscordApplicationCommandOptionChoice(Convert.ToString(i![1]), i[0]));
            }

            foreach (var i in (await BalabobaRequests.GetBalabobaIntros("https://yandex.ru/lab/api/yalm/intros")).Intros!)
            {
